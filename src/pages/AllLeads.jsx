...
export default function AllLeads() {
    // Get the user's plan from localStorage
    const userStr = localStorage.getItem('user');
    const user = userStr ? JSON.parse(userStr) : {};
    const isFreePlan = user.workspacePlan === 'free' || !user.workspacePlan; // Default to free if unknown
    ...
    const columns = [
        ...
        { key: "source", label: "source", icon: "/icons/d-t.svg", color: "#7367f0" }
    ];
            ...
            if (response.success) {
                ...
                setRows(
                    response.data.map(lead => ({
                        ...
                        number: lead.number,
                        link: lead.link,
                        linkName: lead.linkId?.link_name || (lead.link ? "(Deleted link)" : "(Not from Link)"),
                        status: lead.status,
                        source: lead.source,
                        email: lead.email,
    ...
    const handleImportCSV = async (e) => {
        ...
        const res = await leadAPI.importCSV(file);

        if (res.success) {
            showToast("Lead Successfully Imported", "success");
            fetchLeads();
        };
    }
        ...
        <div className="px-4 mt-6">
            <div className="flex-1 w-full max-w-[1400px] mx-auto mt-10">
                        ...
                        <div className="flex gap-2">
                            ...
                            <div className="flex justify-between  items-center w-full px-3">
                                <div className="flex flex-col">
                                    ...
                                    <p className="text-[var(--text-dark)] text-[.55em] ">{selectedLead?.linkId?.link_name || (selectedLead?.link ? "(Deleted link)" : "(Not from Link)")}</p>
                                </div>
                            </div>
                        </div>
                        <div className="flex gap-2">
                ...
                <div className="flex md:flex-row flex-col justify-between md:items-center mb-6 gap-5">
                    ...
                    <div className="flex gap-4">
                        ...
                        {!isFreePlan && (
                            <>
                                <button
                                    onClick={() => document.getElementById("importLead").click()}
                                    className="flex gap-2 w-max text-[.8em]  cursor-pointer text-[var(--text-light)] font-semibold  items-center justify-center bg-[var(--primary-color)] p-2 rounded-[5px] shadow-[0px_0px_10px_color-mix(in_srgb,var(--primary-color)_50%,rgba(255,255,255,0))] hover:shadow-[0px_0px_10px_color-mix(in_srgb,var(--primary-color)_70%,rgba(255,255,255,0))] translate-y-[-2px] hover:translate-y-[-4px] duration-300"
                                >
                                    <MaskImage url="/icons/import.svg" w="1.5em" h="1.5em" bg="var(--text-light)" />
                                    Import Lead
                                </button>
                                <button
                                    ...
                                    className="flex gap-2 w-max text-[.8em]  cursor-pointer text-[var(--text-light)] font-semibold  items-center justify-center bg-[var(--primary-color)] p-2 rounded-[5px] shadow-[0px_0px_10px_color-mix(in_srgb,var(--primary-color)_50%,rgba(255,255,255,0))] hover:shadow-[0px_0px_10px_color-mix(in_srgb,var(--primary-color)_70%,rgba(255,255,255,0))] translate-y-[-2px] hover:translate-y-[-4px] duration-300"
                                >
                                    <MaskImage url="/icons/export.svg" w="1.5em" h="1.5em" bg="var(--text-light)" />
                                    Export Lead
                                </button>
                            </>
                ...
                <div id="all-leads" className="all-leads-wrapper bg-[var(--bg-w)] w-full rounded-[6px] shadow-[0px_0px_10px_rgba(0,0,0,0.1)]  text-[20px]">
                    <form className="flex flex-col shadow-[0px_0px_10px_rgba(0,0,0,0.1)] p-5">
                        ...
                        <div className="grid grid-cols-10 gap-4 mt-5 items-end pb-3 ">
                            ...
                            <div className="col-span-12 md:col-span-2">
                                <div className="flex flex-col gap-1 w-full">
                                    <div className="flex items-end gap-1">
                                        <label htmlFor="" className="text-[.58em] text-[var(--text-dark)] font-bold uppercase leading-4">Source</label>
                                    </div>
                                    <div className="flex w-full gap-2 ">
                                        <div className="relative w-full">
                                            <select
                    ...
                    <div className="bg-[var(--bg-w)] lg:px-5  pb-0">
                        {loading ? (
                            <div className="flex items-center justify-center h-[60vh]">
                                <div className="text-center">
                                    <div className="text-[var(--text-dark)] text-[1.2em] mb-4">Loading leads...</div>
                                    <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-[var(--primary-color)] mx-auto"></div>
                                </div>
                            </div>
                        ) : rows.length === 0 ? (
                            <div className="text-center py-12 flex flex-col items-center">
                                <p className="text-[var(--text-dark)] text-[1em] mb-4">No leads found</p>

                            </div>
                        ) : (
                            <>
                                <div className="lead-data-content-wrapper text-[15px] overflow-x-auto w-full ">
                                    <div className="heading grid grid-cols-[48px_70px_150px_repeat(8,180px)] items-center min-w-[1400px] w-max border-t border-[var(--hover)]">
                                        <div className="sticky left-0 bg-[var(--bg-w)] z-10">
                                            <div className="flex gap-2 items-center py-4 px-2 bg-[var(--bg-w)]">
                                                <input
                                                    type="checkbox"
                                    ...
                                    {rows.map(data => (
                                        <div
                                            ...
                                            className={`grid grid-cols-[48px_70px_150px_repeat(8,180px)] relative items-center min-w-[1400px] w-max pe-5 cursor-pointer ${leadData == data.id ? "bg-[color-mix(in_srgb,var(--primary-color)_5%,#ffffff00)]" : ""}  hover:bg-[color-mix(in_srgb,var(--primary-color)_5%,#ffffff00)]`}>
                                            ...
                                            <div className="border-t border-[var(--hover)]">
                                                <div className="flex gap-2 items-center py-4 px-5 min-w-0">
                                                    <span className="text-[var(--text-dark)] text-[.85em] capitalize truncate" title={data.source}>{data.source}</span>
                                                </div>
                                            </div>
                                        </div>
                                    ))}
