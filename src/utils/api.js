...
const authFetch = async (url, options = {}) => {
  try {
    ...
    if (!response.ok) {
      ...
      // Try to extract JSON error message from backend
      try {
        ...
        // Ignore JSON parsing errors, keep default errorMessage
      }
...
export const authAPI = {
  ...
  googleLogin: async (idToken, actionType = 'login') => {
    const response = await fetch(`${API_BASE_URL}/auth/google-login`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ token: idToken, actionType })
    });
    const data = await response.json();
    if (data.success && data.data.token) {
      setToken(data.data.token);
...
export const linkAPI = {
  ...
  create: async (data) => {
    try {
      ...
      return { success: false, message: error.message || "Failed to create link" };
    }
  },
...
export const leadAPI = {
  ...
  importCSV: async (file) => {
    ...
    const response = await authFetch(`${API_BASE_URL}/leads/import`, {
      method: "POST",
      body: formData
    });
...
export const formAPI = {
  ...
  create: async (data) => {
    try {
      ...
      return { success: false, message: error.message || 'Failed to create form' };
    }
  },
  update: async (id, data) => {
    try {
...
export const contactCardAPI = {
  ...
  create: async (data) => {
    try {
      ...
      // Try to parse the error message if it is an HTTP failure
      return { success: false, message: error.message || "Failed to create contact card" };
    }
  },
...
export const paymentAPI = {
  createOrder: async (planType, amount) => {
    try {
  ...
  verifyPayment: async (paymentData) => {
    try {
      ...
      return { success: false, message: 'Failed to verify payment signature.' };
    }
  }
};