...
import { checkPlanLimits, enforceFormLimit } from '../middleware/workspaceLimit.js';
...
// Create new form
router.post('/', protect, checkPlanLimits, enforceFormLimit, async (req, res) => {
  try {
    const { name, description, title, paragraph, buttonColor, buttonContent, fields, topHeading, offersHeading, offers, isActive } = req.body;
    ...
    const newForm = new Form({
      ...
      buttonContent: buttonContent || 'Unlock My Offer',
      offersHeading: offersHeading || "What You'll Get",
      fields,
      offers,
      topHeading,
...
router.put('/:id', protect, async (req, res) => {
  try {
    const { name, description, title, paragraph, buttonColor, buttonContent, fields, offers, topHeading, offersHeading, isActive, inTrash } = req.body;
    ...
    if (buttonContent !== undefined) updateData.buttonContent = buttonContent;
    if (offersHeading !== undefined) updateData.offersHeading = offersHeading;
    if (fields) updateData.fields = fields;
    if (offers) updateData.offers = offers;
    if (topHeading) updateData.topHeading = topHeading;
...
router.delete('/:id', protect, async (req, res) => {
  try {
    const form = await Form.findOne({ _id: req.params.id, workspaceId: req.user.workspaceId });
...
router.delete('/:id/permanent', protect, async (req, res) => {
  try {
    const deletedForm = await Form.findOneAndDelete({ _id: req.params.id, workspaceId: req.user.workspaceId });
    ...
    if (workspace && workspace.formsUsed > 0) {
      workspace.formsUsed -= 1;
...
router.delete('/clear/all', protect, async (req, res) => {
  try {
    const result = await Form.deleteMany({ workspaceId: req.user.workspaceId });
