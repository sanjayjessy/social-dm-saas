...
import { checkPlanLimits, enforceContactCardLimit } from '../middleware/workspaceLimit.js';
import fs from "fs";
...
router.get("/:id", protect, async (req, res) => {
    try {
        const card = await ContactCard.findOne({ _id: req.params.id, workspaceId: req.user.workspaceId });
...
// Create contact card
router.post("/", protect, checkPlanLimits, enforceContactCardLimit, async (req, res) => {
    try {
        ...
        req.workspace.profileCardsUsed += 1;
...
router.put("/:id", protect, async (req, res) => {
    try {
        ...
        const updatedCard = await ContactCard.findOneAndUpdate(
...
router.put("/:id/avatar", protect, uploadContactAvatar.single("avatar"), async (req, res) => {
    try {
        const card = await ContactCard.findOne({ _id: req.params.id, workspaceId: req.user.workspaceId });
        if (!card) {
            return res.status(404).json({ success: false, message: "Card not found" });
...
router.delete("/:id/permanent", protect, async (req, res) => {
    try {
        const deletedCard = await ContactCard.findOneAndDelete({ _id: req.params.id, workspaceId: req.user.workspaceId });

        if (!deletedCard) {
            return res.status(404).json({ success: false, message: "Contact card not found" });
        }

        // Find the user's workspace and decrement the counter if it's > 0
        const workspace = await mongoose.model("Workspace").findById(req.user.workspaceId);
        if (workspace && workspace.profileCardsUsed > 0) {
            workspace.profileCardsUsed -= 1;
