...
import { checkPlanLimits, enforceLinkLimit } from '../middleware/workspaceLimit.js';
...
router.get('/all/by-date', protect, async (req, res) => {
    try {
...
router.get('/:id', protect, async (req, res) => {
    try {
        ...
        const link = await Link.findOne({ _id: req.params.id, workspaceId: req.user.workspaceId })
            .populate('formId', 'name description fields');
...
// Create new link
router.post('/', protect, checkPlanLimits, enforceLinkLimit, async (req, res) => {
    try {
...
router.delete('/:id', protect, async (req, res) => {
    try {
        const deletedLink = await Link.findOneAndDelete({ _id: req.params.id, workspaceId: req.user.workspaceId });
        ...
        if (workspace && workspace.dmLinksUsed > 0) {
            workspace.dmLinksUsed -= 1;
...
router.get('/slug/:slug', protect, async (req, res) => {
    try {
        const link = await Link.findOne({ slug: req.params.slug, workspaceId: req.user.workspaceId }).populate('formId', 'name description fields title paragraph buttonColor');
