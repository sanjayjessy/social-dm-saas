...
router.get('/stats', protect, async (req, res) => {
  try {
    ...
    const totalLeads = await Lead.countDocuments({ workspaceId: req.user.workspaceId });
    const totalLinks = await Link.countDocuments({ workspaceId: req.user.workspaceId });
...
router.get('/stats/by-date', protect, async (req, res) => {
  try {
...
router.get('/clicks-leads', protect, async (req, res) => {
  try {
    const { startDate, endDate } = req.query;

    const dateQuery = { workspaceId: new mongoose.Types.ObjectId(req.user.workspaceId) };
...
router.get('/page-views', protect, async (req, res) => {
  try {
